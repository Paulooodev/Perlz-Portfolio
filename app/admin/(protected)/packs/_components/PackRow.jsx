...
import { deletePack, togglePublish } from "../_actions";

const PackRow = ({pack, publicUrl}) => {
    ...
    const [isPending, startTransition] = useTransition();
           ...
           const handleTogglePublish = () => {
            ...
            startTransition(async () => {
                const result = await togglePublish(pack.id, pack.is_published);
            ...
            <div
                ...
                transition-colors
                ${pack.is_published ? "border-white/5" : "border-white/5 opacity-60"}
            `}
            >
                ...
                <div className="flex-1 min-w-0">
                    <h3 className="text-white font-bold truncate">{pack.title}</h3>
                    <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                       {pack.subtitle && <span className="truncate">{pack.subtitle}</span>}
                        {pack.sample_count && (
                            <>
                              {pack.subtitle && <span>·</span>}
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                    ...
                    disabled={isPending}
                    title={pack.is_published ? "Unpublish" : "Publish"}
                ...
                >
                    {pack.is_published ? <Eye size={16} /> : <EyeOff size={16} />}
                </button>
